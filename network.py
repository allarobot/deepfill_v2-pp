...
import paddle.nn.initializer as initial
import init
...
def weights_init(net, init_type = 'kaiming', init_gain = 0.02):
    ...
    def init_func(m):
        ...
        if hasattr(m, 'weight') and classname.find('Conv') != -1:
            ...
            if init_type == 'normal':
                init.normal_(m.weight,mean=0.0, std=init_gain)
            elif init_type == 'xavier':
                init.xavier_normal_(m.weight, gain = init_gain)
            elif init_type == 'kaiming':
                init.kaiming_normal_(m.weight, a = 0, mode = 'fan_in')
            elif init_type == 'orthogonal':
                # todo, add orthogonal inilitializer
...
class GatedGenerator(nn.Layer):
    ...
    def forward(self, img, mask):
        ...
        first_in = paddle.concat((first_masked_img, mask), 1)       # in: [B, 4, H, W]
        first_out = self.coarse(first_in)                       # out: [B, 3, H, W]
        # Refinement
        second_masked_img = img * (1 - mask) + first_out * mask
        second_in = paddle.concat((second_masked_img, mask), 1)     # in: [B, 4, H, W]

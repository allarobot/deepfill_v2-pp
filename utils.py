...
def load_dict(process_net, pretrained_net):
    ...
    process_net.set_state_dict(process_dict)
    return process_net
...
def save_sample_png(sample_folder, sample_name, img_list, name_list, pixel_max_cnt = 255):
    ...
    for i in range(len(img_list)):
        ...
        img_copy = img.clone().transpose([0, 2, 3, 1])[0, :, :, :].numpy()
...
def psnr(pred, target, pixel_max_cnt = 255):
    mse = paddle.mul(target - pred, target - pred)
    rmse_avg = (paddle.mean(mse).item()) ** 0.5
    p = 20 * np.log10(pixel_max_cnt / rmse_avg)
    return p

def grey_psnr(pred, target, pixel_max_cnt = 255):
    pred = paddle.sum(pred, dim = 0)
    target = paddle.sum(target, dim = 0)
    mse = paddle.mul(target - pred, target - pred)
    rmse_avg = (paddle.mean(mse).item()) ** 0.5
...
def ssim(pred, target):
    pred = pred.clone().transpose([0, 2, 3, 1]).numpy()
    target = target.clone().transpose([0, 2, 3, 1]).numpy()
    target = target[0]
    pred = pred[0]
